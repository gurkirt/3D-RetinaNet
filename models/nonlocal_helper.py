...
class Nonlocal(nn.Module):
    ...
    def _construct_nonlocal(self, zero_init_final_conv, zero_init_final_norm):
        ...
        c2_msra_fill(self.conv_g)
        c2_msra_fill(self.conv_phi)
        c2_msra_fill(self.conv_theta)
        # Zero initializing the final convolution output.
        self.conv_out.zero_init = zero_init_final_conv
        c2_msra_fill(self.conv_out)
        ...
        if self.norm_type == "batchnorm":
            ...
            batchnorm_weight = float(zero_init_final_norm)
            self.bn.weight.data.fill_(0.0)
    ...
    def forward(self, x):
        ...
        if self.norm_type == "batchnorm":
            ...
            p = self.ln(p)
        # pdb.set_trace()
        # print('pmean', self.conv_out.weight.shape, p.mean(), p.std())
        return x_identity + p
